function [ber_average] = GRA_BER(selected_numbers)
%GRA_BER 返回要回传数据的无人机平均误码率 误码率需要根据接收信号强度计算 ，所以在计算网络的时候还需要重新计算一次r's'srss
...
for i = 101 : uav_rows + 100
    ...
    candiate_net = GRA_CalcCanNet(uav);
    ...
    % 如果上面都不满足
    if ~flag
        target_net = sortrows(candiate_net, 7, "descend");
