...
for i = 1 : 4
    ...
    % 自己的算法
    disp('-------------BlQoeE-------------');
    BSLoadDegree_total(1, i) = BLQoE_BER(selected_numbers);
    
    % 对比算法1 ahpsaw
     disp('------------AHPSAW-------------');
    BSLoadDegree_total(2, i) = AHPSAW_BER(selected_numbers);
   

    % 对比算法2 灰色关联GRA
    disp('------------------GRA----------------');
    BSLoadDegree_total(3, i) = GRA_BER(selected_numbers);
    

    % 对比算法3 RSR
    BSLoadDegree_total(4, i) = RSR_BER(selected_numbers);
...
writematrix(BSLoadDegree_total,'D:\simulation\BER\BER.xlsx','Sheet','BER', 'WriteMode','replacefile');
